...
class Utils {
  ...
  static <K, V> LinkedHashMap<K, V> mergeLeft(
      LinkedHashMap<K, V> f, LinkedHashMap<K, V> f2, LinkedHashMap<K, V> f3) {
  ...
  private static final Set<String> CssProperties =
      Set.of(
          "background",
  ...
  static boolean isCssProperty(String propertyName) {
    if (isNullOrWhiteSpace(propertyName)) return false;
  ...
  static String suffixCssClasses(String cssClasses, String suffix) {
    if (isNullOrWhiteSpace(cssClasses) || isNullOrWhiteSpace(suffix)) return "";
  ...
  static <T> LinkedHashMap<String, T> mapOf(
      String k, T v, String k2, T v2, String k3, T v3, String k4, T v4) {
    var res = mapOf(k, v, k2, v2, k3, v3);
    res.put(k4, v4);
    return res;
  }

  static LinkedHashMap<String, String> mapOf(
      String k, String v, String k2, String v2, String... rest) {
    var res = mapOf(k, v, k2, v2);
  ...
  static StringBuilder msoConditionalTag(CharSequence content, boolean negation) {
    return new StringBuilder(negation ? startMsoNegationConditionalTag : startMsoConditionalTag)
        .append("\n")
        .append(content)
        .append((negation ? endNegationConditionalTag : endConditionalTag))
        .append("\n");
  }
  ...
  static StringBuilder conditionalTag(CharSequence content, boolean negation) {
    return new StringBuilder(negation ? startNegationConditionalTag : startConditionalTag)
        .append("\n")
        .append(content)
        .append(negation ? endNegationConditionalTag : endConditionalTag)
        .append("\n");
  ...
  private static final Pattern FIND_OUTLOOK_CONDITIONALS =
      Pattern.compile("(<!\\[endif]-->\\s*?<!--\\[if mso \\| IE]>)", Pattern.MULTILINE);

  static String mergeOutlookConditionals(CharSequence seq) {
    return FIND_OUTLOOK_CONDITIONALS.matcher(seq).replaceAll("");
  }

  private static final Pattern FIND_OUTLOOK_CONDITIONALS_MINIFY =
      Pattern.compile("(<!--\\[if\\s[^\\]]+]>)([\\s\\S]*?)(<!\\[endif]-->)", Pattern.MULTILINE);
  private static final Pattern FIND_SPACES_BETWEEN_TAGS =
  ...
  static String minifyOutlookConditionals(CharSequence seq) {
    return FIND_OUTLOOK_CONDITIONALS_MINIFY
        .matcher(seq)
        .replaceAll(
            (match) -> {

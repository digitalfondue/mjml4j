...
class MjmlComponentCarouselImage extends BaseComponent.BodyComponent {
  ...
  boolean hasThumbnailsSupported() {
    var thumbnails =
        hasAttribute("thumbnails") ? getAttribute("thumbnails") : localContext.thumbnails();
  ...
  StringBuilder renderThumbnail(HtmlRenderer renderer) {
    ...
    renderer.openTag(
        ...
        htmlAttributes(
            mapOf(
                ...
                getAttribute("target"),
                "class",
                "mj-carousel-thumbnail mj-carousel-"
                    + carouselId
                    + "-thumbnail mj-carousel-"
                    + carouselId
                    + "-thumbnail-"
                    + imgIndex
                    + " "
    ...
    renderer.openTag(
        "label",
        htmlAttributes(mapOf("for", "mj-carousel-" + carouselId + "-radio-" + imgIndex)),
        res);
    ...
    renderer.openCloseTag(
        ...
        htmlAttributes(
            mapOf(
                ...
                getAttribute("alt"),
                "width",
                doubleToString(CssUnitParser.parse(getAttribute("tb-width")).value()))),
        res);
  ...
  StringBuilder renderMjml(HtmlRenderer renderer) {
            ...
            renderer.openCloseTag(
                ...
                htmlAttributes(
                    mapOf(
                        ...
                        "style", "images_img",
                        "width",
                            doubleToString(getContainerOuterWidth()), // this.context.containerWidth
                        "border", "0")),
                res);
    ...
    renderer.openTag(
        ...
        htmlAttributes(
            mapOf(
                "class",
                "mj-carousel-image mj-carousel-image-"
                    + (index + 1)
                    + " "
                    + (cssClass != null ? cssClass : ""),
                "style",
                index == 0 ? "images_firstImageDiv" : "images_otherImageDiv")),
        res);

    if (href != null) {
      var rel = getAttribute("rel");
      renderer.openTag(
          "a", htmlAttributes(mapOf("href", href, "rel", rel, "target", "_blank")), res, false);
      renderImage.run();
      renderer.closeTag("a", res);
    } else {
      renderImage.run();

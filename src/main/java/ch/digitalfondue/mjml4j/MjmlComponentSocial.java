...
class MjmlComponentSocial extends BaseComponent.BodyComponent {
    ...
    void setupStyles(CssStyleLibraries cssStyleLibraries) {
        cssStyleLibraries.add("tableVertical", mapOf(
                "margin", "0px"
        ));
    }
    ...
    StringBuilder renderMjml(HtmlRenderer renderer) {
        return "horizontal".equals(getAttribute("mode")) ? renderHorizontal(renderer) : renderVertical(renderer);
    }


    private static final Set<String> INHERITING_ATTRIBUTES = Set.of("border-radius",
            "color",
            "font-family",
            "font-size",
            "font-style",

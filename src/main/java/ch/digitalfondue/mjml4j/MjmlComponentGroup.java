...
class MjmlComponentGroup extends BaseComponent.BodyComponent {
  ...
  CssBoxModel getBoxModel() {
    ...
    return new CssBoxModel(
        parsedContextContainerWidth.value(), 0, 0, parsedContextContainerWidth.value());
  }
  ...
  private CssUnitParser.CssParsedUnit getParsedWidth() {
    String width;
    if (hasAttribute("width")) width = getAttribute("width");
    else width = doubleToString(100. / getSectionColumnCount()) + "%";
  ...
  private String getWidthAsPixel() {
    ...
    return parsedWidth.isPercent()
        ? doubleToString(parsedContainerWidth.value() * parsedWidth.value() / 100) + "px"
                    ...
                    mapOf(
                        ...
                        component.inlineCss(
                            mapOf(
                                ...
                                "width",
                                    getElementWidth(
                                        component.hasAttribute("width")
                                            ? component.getAttribute("width")
                                            : ((MjmlComponentColumn) component)
                                                .getWidthAsPixel()))))))
            .append(" >\n");
        sb.append("<![endif]-->\n");
        sb.append(childContent);
  ...
  StringBuilder renderMjml(HtmlRenderer renderer) {
    ...
    if (hasAttribute("css-class")) classesName += " " + getAttribute("css-class");

    res.append("<div ")
        .append(htmlAttributes(mapOf("class", classesName, "style", "div")))
        .append(">\n");
                ...
                mapOf(
                    "bgcolor",
                        hasAttribute("background-color")
                            ? getAttribute("background-color").equals("none")

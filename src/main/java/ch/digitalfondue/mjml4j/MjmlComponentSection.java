...
class MjmlComponentSection extends BaseComponent.BodyComponent {
  ...
  private StringBuilder renderSimple(HtmlRenderer renderer) {
    ...
    return new StringBuilder(renderBefore(renderer))
        .append(
            bHasBackground
                ? renderWithBackground(renderSection(renderer))
  ...
  private static CssCoordinate getBackgroundPositionAsPercentage(
      String backgroundPositionX, String backgroundPositionY) {
  ...
  private StringBuilder renderSection(HtmlRenderer renderer) {
    ...
    renderer.openTag(
        "div",
        htmlAttributes(
            mapOf("class", bIsFullWidth ? null : getAttribute("css-class"), "style", "div")),
        res);
    if (bHasBackground) {
  ...
  void setupStyles(CssStyleLibraries cssStyleLibraries) {
    ...
    cssStyleLibraries.add(
        "tableFullwidth",
        isFullWidth ? mergeLeft(background, mapOf("width", "100%")) : mapOf("width", "100%"));

    var tableBaseMap = mapOf("width", "100%");
    if (hasBorderRadius()) {
      tableBaseMap.put("border-collapse", "separate");
    }
    cssStyleLibraries.add(
        "table", !isFullWidth ? mergeLeft(background, tableBaseMap) : tableBaseMap);
    ...
    var baseDivStyles =
        mapOf(
            ...
            "0px auto",
            "max-width",
            doubleToString(getContainerOuterWidth()) + "px",
            "border-radius",
            getAttribute("border-radius"));
    if (hasBorderRadius()) {
    ...
    cssStyleLibraries.add(
        "div", isFullWidth ? baseDivStyles : mergeLeft(background, baseDivStyles));
  ...
  private CssCoordinate parseBackgroundPosition() {
    ...
    if (posSplit.length == 2) {
      ...
      if (value1.equalsIgnoreCase("top")
          ...
          || (value1.equalsIgnoreCase("center") && value2.equalsIgnoreCase("left"))

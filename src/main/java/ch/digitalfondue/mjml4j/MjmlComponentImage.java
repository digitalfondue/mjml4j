...
class MjmlComponentImage extends BaseComponent.BodyComponent {
  ...
  void setupStyles(CssStyleLibraries cssStyleLibraries) {
    ...
    cssStyleLibraries.add("td", mapOf("width", isFullWidth ? null : width + "px"));
  ...
  String headStyle() {
    return "    @media only screen and (max-width:"
        + makeLowerBreakpoint(context.breakpoint)
        + ") {\n"
    ...
    renderer.openCloseTag(
        ...
        htmlAttributes(
            mapOf(
                ...
                "height",
                    bHasHeight && height.equalsIgnoreCase("auto")
                        ? height
                        : doubleToString(CssUnitParser.parse(height).value()))),
        res);
    if (hasHref) {
  ...
  StringBuilder renderMjml(HtmlRenderer renderer) {
    ...
    renderer.openTag(
        "td",
        htmlAttributes(
            mapOf("style", "td", "class", bIsFluidMobile ? "mj-full-width-mobile" : null)),
        res);

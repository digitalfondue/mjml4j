...
class MjmlComponentAccordionElement extends BaseComponent.BodyComponent {
  ...
  void setupStyles(CssStyleLibraries cssStyleLibraries) {
    cssStyleLibraries.add(
        "td", mapOf("padding", "0px", "background-color", getAttribute("background-color")));

    cssStyleLibraries.add(
        "label", mapOf("font-size", "13px", "font-family", getAttribute("font-family")));

    cssStyleLibraries.add("input", mapOf("display", "none"));
  }
  ...
  StringBuilder renderMjml(HtmlRenderer renderer) {
    ...
    renderer.openTag(
        "label", htmlAttributes(mapOf("class", "mj-accordion-element", "style", "label")), res);

    var tmp = new StringBuilder();
    renderer.openCloseTag(
        "input",
        htmlAttributes(
            mapOf("class", "mj-accordion-checkbox", "type", "checkbox", "style", "input")),
        tmp);
